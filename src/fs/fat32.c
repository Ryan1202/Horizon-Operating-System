...
int FAT32_read(struct index_node *inode, uint8_t *buffer, uint32_t length)
{
    ...
    cnt = fp->offset / (SECTOR_SIZE * fat32->BPB_SecPerClus);
    pos = fat_next(inode, fp->start, cnt, 0);
    dev->f_ops.seek(dev, (fp->offset / SECTOR_SIZE) % fat32->BPB_SecPerClus + fat32->data_start + (pos - 2) * fat32->BPB_SecPerClus, 0);
    if (fp->offset % (SECTOR_SIZE * fat32->BPB_SecPerClus)) {
        tmp = SECTOR_SIZE * fat32->BPB_SecPerClus - fp->offset % (SECTOR_SIZE * fat32->BPB_SecPerClus);
        dev->f_ops.seek(dev, fat32->data_start + (pos - 2) * fat32->BPB_SecPerClus, 0);
        dev->f_ops.read(dev, (uint8_t *)buf, tmp);
        memcpy(buffer, buf + fp->offset % (SECTOR_SIZE * fat32->BPB_SecPerClus), tmp);
        off += tmp;
    }
    dev->f_ops.read(dev, buffer + off, length - off);
    tmp = fp->offset;
    fp->offset += length;
...
int FAT32_write(struct index_node *inode, uint8_t *buffer, uint32_t length)
{
    ...
    pos = fat_next(inode, fp->start, cnt, 1);
    ...
    if (fp->offset % SECTOR_SIZE + length <= SECTOR_SIZE)
        goto done;
    else {
    ...
    dev->f_ops.seek(dev, fat32->data_start + (pos - 2) * fat32->BPB_SecPerClus + fp->dir_offset / SECTOR_SIZE, 0);
    dev->f_ops.read(dev, (uint8_t *)buf, SECTOR_SIZE);
    sdir = (struct FAT32_dir *)(buf + fp->dir_offset % SECTOR_SIZE);

    inode->last_access_date.year
        = inode->write_date.year  = BCD2BIN(CMOS_READ(CMOS_YEAR));
...
struct index_node *FAT32_create_file(partition_t *part, struct index_node *parent, char *name, int len)
{
    ...
    do {
        ...
        if (i / SECTOR_SIZE / fat32->BPB_SecPerClus)
            pos = fat_next(inode, pos, 1, 1);
    } while (buf[i]);
    ...
    if (len > 11 || len_ext > 3 || len_without_ext > 8 || (flag & 0x03) == 0x03 || (flag & 0x0c) == 0x0c) {
        ...
        for (i = 0; i < 8; i++) {
            if (i < name_len)
                filename_short[i] = toupper(name[i] ? name[i] : ' ');
            else if (i == name_len)
                filename_short[i] = '~';
            else if (i - name_len < count)
                filename_short[i] = (name_count / tmp % 10 + '0');
            else
                filename_short[i] = ' ';
            tmp /= 10;
        }
        ...
        sdir->DIR_NTRes = 0;
        if (flag & 0x01)
            sdir->DIR_NTRes |= FAT32_BASE_L;
        if (flag & 0x04)
            sdir->DIR_NTRes |= FAT32_EXT_L;
...
struct index_node *FAT32_open(struct _partition_s *part, struct index_node *parent, char *filename)
{
    ...
    while (f) {
        if (find_member_in_fat(part, cc) >= 0x0fffffff)
            f = 0;
        ...
        for (i = 0x00; i < SECTOR_SIZE * fat32->BPB_SecPerClus; i += 0x20) {
            if (buf[i + 11] == FAT32_ATTR_LONG_NAME)
                continue;
            if (buf[i] == 0xe5 || buf[i] == 0x00 || buf[i] == 0x05)
                continue;
            ...
            while (ldir->LDIR_Attr == FAT32_ATTR_LONG_NAME && ldir->LDIR_Ord != 0xe5) {
                for (x = 0; x < 5; x++) {
                    if (j > len && ldir->LDIR_Name1[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name1[x] != (unsigned short)(filename[j++]))
                        goto cmp_fail;
                }
                for (x = 0; x < 6; x++) {
                    if (j > len && ldir->LDIR_Name2[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name2[x] != (unsigned short)(filename[j++]))
                        goto cmp_fail;
                }
                for (x = 0; x < 2; x++) {
                    if (j > len && ldir->LDIR_Name3[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name3[x] != (unsigned short)(filename[j++]))
                        goto cmp_fail;
                }
        ...
        for (i = 0x00; i < SECTOR_SIZE * fat32->BPB_SecPerClus; i += 0x20) {
            ...
            while (ldir->LDIR_Attr == FAT32_ATTR_LONG_NAME && ldir->LDIR_Ord != 0xe5) {
                for (x = 0; x < 5; x++) {
                    if (j > len && ldir->LDIR_Name1[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name1[x] != (unsigned short)(name[j++]))
                        goto cmp_fail;
                }
                for (x = 0; x < 6; x++) {
                    if (j > len && ldir->LDIR_Name2[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name2[x] != (unsigned short)(name[j++]))
                        goto cmp_fail;
                }
                for (x = 0; x < 2; x++) {
                    if (j > len && ldir->LDIR_Name3[x] == 0xffff)
                        continue;
                    else if (j > len || ldir->LDIR_Name3[x] != (unsigned short)(name[j++]))
                        goto cmp_fail;
                }
...
uint32_t fat_next(struct index_node *inode, uint32_t clus, int next, int alloc)
{
    ...
    if (inode->fp->cur_pos - inode->fp->start <= clus) {
        c = c - (inode->fp->cur_pos - inode->fp->start);
    }
