...
FatDirEntry *generate_dir_entry(
	struct FatInfo *fat_info, struct FatDirEntry *parent_entry,
	ShortDir *short_dir, string_t name, struct FatLocation *location,
	ObjectAttr *attr);
...
FsResult fat_delete_entry(
	struct FatInfo *fat_info, FatDirEntry *parent,
	struct FatLocation *location);
