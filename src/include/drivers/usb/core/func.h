...
#define usb_get_standard_descriptor(                                    \
	hcd, device, recipient, index, value_hi, value_lo, desc_size)       \
	usb_get_descriptor(                                                 \
		hcd, device, USB_REQ_TYPE_STANDARD, recipient, index, value_hi, \
		value_lo, desc_size)
#define usb_get_class_descriptor(                                              \
	hcd, device, recipient, index, value_hi, value_lo, desc_size)              \
	usb_get_descriptor(                                                        \
		hcd, device, USB_REQ_TYPE_CLASS, recipient, index, value_hi, value_lo, \
		desc_size)
#define usb_get_vendor_descriptor(                                    \
	hcd, device, recipient, index, value_hi, value_lo, desc_size)     \
	usb_get_descriptor(                                               \
		hcd, device, USB_REQ_TYPE_VENDOR, recipient, index, value_hi, \
		value_lo, desc_size)

#define usb_get_device_descriptor(hcd, device)                             \
	usb_get_standard_descriptor(                                           \
		hcd, device, USB_REQ_RECIPIENT_DEVICE, 0, USB_DESC_TYPE_DEVICE, 0, \
		sizeof(struct UsbDeviceDescriptor))
#define usb_get_interface_descriptor(hcd, device)                             \
	usb_get_standard_descriptor(                                              \
		hcd, device, USB_REQ_RECIPIENT_DEVICE, 0, USB_DESC_TYPE_INTERFACE, 0, \
...
struct UsbConfigDescriptor *usb_get_config_descriptor(
	UsbHcd *hcd, struct UsbDevice *device);

uint8_t		   usb_get_config(UsbHcd *hcd, struct UsbDevice *device);
UsbSetupStatus usb_set_config(
	UsbHcd *hcd, struct UsbDevice *device, uint8_t config);

struct UsbHubDescriptor *usb_get_hub_descriptor(UsbHub *hub);

UsbSetupStatus usb_set_address(
	UsbHcd *hcd, struct UsbDevice *device, uint32_t address);
...
struct UsbStringDescriptor *usb_get_string_descriptor(
	UsbHcd *hcd, struct UsbDevice *device, uint8_t index);

void usb_show_device_descriptor(
	UsbHcd *hcd, struct UsbDevice *device, struct UsbDeviceDescriptor *desc);
