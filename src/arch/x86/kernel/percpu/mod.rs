...
impl CpuLocal for X86CpuLocal {
    fn delta_for(base: *mut u8) -> usize {
        base.addr().wrapping_sub((&raw const __percpu_start).addr())
    }
    ...
    unsafe fn get_ptr_dyn_for<T: PerCpuInit>(percpu: &PerCpuDyn<T>, delta: usize) -> *const T {
        let addr = percpu.get_raw_handle().wrapping_add(delta);
        with_exposed_provenance(addr)
    }
}
...
macro impl_percpu_scalar {
    ($(($ty:ty, $reg:ident, $size:literal $(, $modifier:literal)?)),+ $(,)?) => {
        $(
            ...
            impl $crate::kernel::memory::percpu::PerCpuReadWrite<$ty> for $crate::kernel::memory::percpu::PerCpuDyn<$ty> {
                #[inline(always)]
                fn read(&self) -> $ty {
                    let value: $ty;
                    unsafe {
                        asm!(
                            concat!("mov {value", $(":", $modifier,)? "}, ", $size, " ptr gs:[{offset}]"),
                            value = out($reg) value,
                            offset = in(reg) self.get_raw_handle(),
                            options(nostack, preserves_flags),
                        )
                    }
                    value
            ...
            impl $crate::kernel::memory::percpu::PerCpuScalar<$ty> for $crate::kernel::memory::percpu::PerCpuDyn<$ty> {
                #[inline(always)]
                fn add(&self, value: $ty) {
                    unsafe {
                        asm!(
