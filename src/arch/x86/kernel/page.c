...
void setup_page(void)
{
    ...
    uint32_t  size      = (*(unsigned short *)(VIDEO_INFO_ADDR + 0)) * (*(unsigned short *)(VIDEO_INFO_ADDR + 2)) * (*(uint32_t *)(VIDEO_INFO_ADDR + 4) / 8);
...
int __remap(uint32_t paddr, uint32_t vaddr, size_t size)
{
    ...
    while (vaddr < end) {
        if (__page_link(vaddr, paddr, SIGN_RW))
            return -1;
        vaddr += PAGE_SIZE;
        paddr += PAGE_SIZE;
    }
...
void *thread_alloc_page(struct task_s *thread, int pages)
{
    ...
    if (pages == 1) { // 如果只有一个页
        ...
        for (i = 1; i < pages; i++) {
            ...
            fill_vir_page_table(vir_page_addr_more, SIGN_USER); // 把页添加到当前页目录表系统中，使他可以被使用
        }
        memset((void *)vir_page_addr, 0, PAGE_SIZE * pages);
        return (void *)vir_page_addr;
    }
