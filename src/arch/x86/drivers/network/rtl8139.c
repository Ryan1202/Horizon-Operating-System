...
void rtl8139_handler(int irq) {
	...
	if (status & RTL8139_ISR_ROK) {
		...
		wq = wait_queue_first(devext->wqm);
		rq = (struct read_request_s *)wq->private_data;
		memcpy(rq->buffer, devext->rx_buffer, rq->length);
	}
	return;
}
...
static status_t rtl8139_enter(driver_t *drv_obj) {
	...
	put_irq_handler(devext->irq, (irq_handler_t)rtl8139_handler);
	irq_enable(devext->irq);
