...
void sb16_handler(int irq)
{
	...
	if (devext->major_ver >= 4)
	{
		io_in8(SB16_DESP_IACK16);
	}
...
void sb16_dma_config(uint8_t channel, uint32_t addr, uint32_t length)
{
	...
	dma_set_mode(channel, 0);
	dma_set_addr(channel, addr);
	dma_set_count(channel, length - 1);
	io_out8(DMA_PAGE5, (char)(addr>>16));
	dma_enable(channel);
}
...
status_t sb16_write(device_t *dev, uint8_t *buf, uint32_t offset, size_t size)
{
	...
	int i;
	if ((devext->index_w + 1)%DMA_MAX == devext->index_r)
	{
