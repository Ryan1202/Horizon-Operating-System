...
void write_pixel(int x, int y, unsigned int color) {
	...
	unsigned char *vram =
		(unsigned char *)(video_info.vram + (y * video_info.width + x) *
												(video_info.BitsPerPixel / 8));
	if (video_info.BitsPerPixel == 32) {
		vram[0] = color & 0xff;
		vram[1] = (color >> 8) & 0xff;
		vram[2] = (color >> 16) & 0xff;
	} else if (video_info.BitsPerPixel == 24) {
		vram[0] = color & 0xff;
		vram[1] = (color >> 8) & 0xff;
		vram[2] = (color >> 16) & 0xff;
	} else if (video_info.BitsPerPixel == 16) {
		vram[0] = ((g * 63 / 255) & 0x7) << 5 | (b * 31 / 255);
		vram[1] = (r * 31 / 255) << 3 | ((g * 63 / 255) & 0x38) >> 3;
	} else if (video_info.BitsPerPixel == 8) {
...
void print_word(int x, int y, unsigned char *ascii, unsigned int color) {
	...
	char d;
	for (i = 0; i < 16; i++) {
		// vram = (unsigned char *)(video_info.vram + ((y+i)*video_info.width +
		// x)*(video_info.BitsPerPixel/8));
		d = ascii[i];
