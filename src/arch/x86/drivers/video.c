...
void print_word(int x, int y, unsigned char *ascii, unsigned int color)
{
    ...
    for (i = 0; i < 16; i++) {
        ...
        d = ascii[i];
        if (d & 0x80) {
            write_pixel(x + 0, y + i, color);
        }
        if (d & 0x40) {
            write_pixel(x + 1, y + i, color);
        }
        if (d & 0x20) {
            write_pixel(x + 2, y + i, color);
        }
        if (d & 0x10) {
            write_pixel(x + 3, y + i, color);
        }
        if (d & 0x08) {
            write_pixel(x + 4, y + i, color);
        }
        if (d & 0x04) {
            write_pixel(x + 5, y + i, color);
        }
        if (d & 0x02) {
            write_pixel(x + 6, y + i, color);
        }
        if (d & 0x01) {
            write_pixel(x + 7, y + i, color);
        }
    }
}
