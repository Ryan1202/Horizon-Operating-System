 ...
 * @date 2022-09-10
 * 
 * @copyright Copyright (c) 2022 Ryan Wang
 * 
...
void uhci_print_status(device_extension_t *devext)
{
	...
	if (status&0x20)
	{
		printk("[UHCI]HCHalted.\n");
	}
	if (status&0x10)
	{
		printk("[UHCI]HC Process Error.\n");
	}
	if (status&0x08)
	{
		printk("[UHCI]Host System Error.\n");
	}
	if (status&0x04)
	{
		printk("[UHCI]Resume Detect.\n");
	}
...
static status_t uhci_enter(driver_t *drv_obj)
{
	...
	devext->device = pci_get_device_ByClass(UHCI_CLASSID, UHCI_SUBCLASSID, UHCI_PROGIF);
	...
	io_out16(devext->io_base+UHCI_REG_USBINTR, intr | UHCI_INTR_SPI | UHCI_INTR_IOC | UHCI_INTR_RESUME | UHCI_INTR_CRC);
	...
	// put_irq_handler(devext->device->irqline, uhci_handler);
