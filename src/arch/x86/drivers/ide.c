...
static status_t ide_enter(driver_t *drv_obj)
{
    ...
    if (device->prog_if & 0x01) {
        if (device->prog_if & 0x02) {
            pci_write8(device->bus, device->dev, device->function,
                       PCI_REG_PROGIF, device->prog_if | 0x01);
            device->prog_if = pci_read8(device->bus, device->dev, device->function,
                                        PCI_REG_PROGIF);
        } else {
            return UNSUPPORT;
        }
    ...
    if (device->prog_if & 0x04) {
        if (device->prog_if & 0x08) {
            pci_write8(device->bus, device->dev, device->function,
                       PCI_REG_PROGIF, device->prog_if | 0x04);
            device->prog_if = pci_read8(device->bus, device->dev, device->function,
                                        PCI_REG_PROGIF);
        } else {
            return UNSUPPORT;
...
{
#ifdef IDE_DMA_MODE
    if (mode == 0 && rw == IDE_READ)
        *cmd = ATA_CMD_READ_DMA;
    if (mode == 1 && rw == IDE_READ)
        *cmd = ATA_CMD_READ_DMA;
    if (mode == 2 && rw == IDE_READ)
        *cmd = ATA_CMD_READ_DMA_EXT;
    if (mode == 0 && rw == IDE_WRITE)
        *cmd = ATA_CMD_WRITE_DMA;
    if (mode == 1 && rw == IDE_WRITE)
        *cmd = ATA_CMD_WRITE_DMA;
    if (mode == 2 && rw == IDE_WRITE)
