...
uint8_t usb_get_config(UsbHcd *hcd, UsbDevice *device) {
	uint8_t config;
...
UsbSetupStatus usb_set_config(UsbHcd *hcd, UsbDevice *device, uint8_t config) {
...
struct UsbStringDescriptor *usb_get_string_descriptor(
	UsbHcd *hcd, UsbDevice *device, uint8_t index) {
	uint8_t buffer[2];
...
UsbSetupStatus usb_set_address(
	UsbHcd *hcd, UsbDevice *device, uint32_t address) {
	...
	if (status == USB_SETUP_SUCCESS) device->address = address;
	return status;
}
...
void usb_show_device_descriptor(
	UsbHcd *hcd, UsbDevice *device, struct UsbDeviceDescriptor *desc) {
	...
	struct UsbStringDescriptor *str1 =
		usb_get_string_descriptor(hcd, device, desc->iManufacturer);
	...
	struct UsbStringDescriptor *str2 =
		usb_get_string_descriptor(hcd, device, desc->iProduct);
	...
	struct UsbStringDescriptor *str3 =
		usb_get_string_descriptor(hcd, device, desc->iSerialNumber);
