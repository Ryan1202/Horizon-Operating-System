...
// seek(prog->inode, 0, 0);
// read(prog->inode, (uint8_t *)&header, sizeof(struct elf32_header));

// if (memcmp(header.e_ident, "\177ELF\1\1\1", 7) ||
// 	header.e_type != ELF32_ET_EXEC || header.e_machine != ELF32_EM_386 ||
// 	header.e_version != 1 || header.e_phnum > 1024 ||
...
// seek(prog->inode, prog_header_off, 0);
// read(prog->inode, (uint8_t *)&pheader, prog_header_size);
