...
DriverResult register_bus_driver(
	...
	bus_driver->new_device_num = 0;
	bus_driver->device_count   = 0;
...
DriverResult create_bus(Bus **bus, BusDriver *bus_driver, BusOps *ops) {
	*bus = kmalloc(sizeof(Bus));
	...
	string_new(&b->name, _name, sizeof(_name));
	...
	b->object = create_object_directory(
		bus_driver->object, &b->name, base_obj_sys_attr);
