...
PRIVATE void md5_transform(uint32_t state[4], const uint8_t block[64]) {
	uint32_t a = state[0], b = state[1], c = state[2], d = state[3];
	uint32_t x[16];

	for (int i = 0; i < 16; i++) {
		x[i] = HOST2LE_DWORD(((uint32_t *)block)[i]);
	}
...
void md5(uint8_t digest[16], const uint8_t *data, size_t len) {
	...
	for (size_t i = 0; i < 4; i++) {
		((uint32_t *)digest)[i] = LE2HOST_DWORD(state[i]);
	}
}