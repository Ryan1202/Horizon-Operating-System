...
void move_layer(struct gui_s *gui, struct layer_s *layer, int new_x, int new_y) {
	...
	if (gui->update_area.updated == 0) {
		...
		gui->update_area.newRect.l = MIN(new_x, gui->update_area.newRect.l);
		gui->update_area.newRect.t = MIN(new_y, gui->update_area.newRect.t);
		gui->update_area.newRect.r = MAX(layer->rect.r, gui->update_area.newRect.r);
		gui->update_area.newRect.b = MAX(layer->rect.b, gui->update_area.newRect.b);
	} else {
		...
		gui->update_area.newRect.l = new_x;
		gui->update_area.newRect.t = new_y;
		gui->update_area.newRect.r = layer->rect.r;
