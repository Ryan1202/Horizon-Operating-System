...
void draw_win(struct win_s *win) {
	...
	g_print_string(win->layer, width / 2 - win->title.length * 9 / 2, 7, font16, win->title.text,
				   WIN_FRONT_COLOR);
	...
	for (i = 0; i < 10; i++) {
		for (j = 0; j < 10; j++) {
			if (close_btn[i][j] == 'o') {
				((uint32_t *)win->layer->buffer)[((i + 10) * width + j + width - 20) * win->layer->bpp / 4] =
					WIN_FRONT_COLOR;
...
void close_btn_trigger(struct win_s *win, uint32_t value, uint32_t *private_data) {
	...
	int			width		 = win->layer->rect.r - win->layer->rect.l;
	struct Rect refresh_rect = {win->layer->rect.r - 30, win->layer->rect.r, win->layer->rect.t,
								win->layer->rect.t + 30};
...
void close_win(struct win_s *win) {
	...
	if (win->gui->focus == win->layer) { win->gui->focus = win->gui->bg; }
...
void default_win_handler(struct win_s *win, uint32_t msgtype, uint32_t value1, uint32_t value2) {
	...
	case WIN_MSG_RCLICK:
		list_for_each_owner_safe (cur, next, &win->triggers.list_head, list) {
			if (cur->type != msgtype) { continue; }
			if (cur->range.l <= x && x <= cur->range.r && cur->range.t <= y && y <= cur->range.b) {
				cur->triggered		  = 1;
